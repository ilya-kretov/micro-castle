...
//    }
    ...
    public void touchAggregator() {
        System.out.println("Pinging aggregator");
        ResponseEntity<String> helloResponse = restTemplate.getForEntity("http://" + aggregatorHost + ":8080/hello", String.class);
        if (helloResponse.getStatusCode().is2xxSuccessful()) {
            System.out.println(helloResponse.getBody());
        }
        else {
            System.out.println("Failed to receive response " + helloResponse.getStatusCode());
        }
    }
